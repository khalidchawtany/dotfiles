...
debug = function(message) {
  return Phoenix.alert(message, 10);
};
...
changeGridWidth = function(by_) {
  ...
  Phoenix.alert("grid is now " + GRID_WIDTH + " tiles wide", 1);
  snapAllToGrid();
};

changeGridHeight = function(by_) {
  GRID_HEIGHT = Math.max(1, GRID_HEIGHT + by_);
  Phoenix.alert("grid is now " + GRID_HEIGHT + " tiles high", 1);
  snapAllToGrid();
};
...
Window.prototype.getGrid = function() {
  ...
  winFrame = this.frame();
  screenRect = this.screen().visibleFrameInRectangle();
...
Window.prototype.calculateGrid = function(x, y, width, height) {
  var screen;
  screen = this.screen().visibleFrameInRectangle();
  return {
...
Window.prototype.toMidCenter = function() {
  return this.toGrid(0.1, 0.05, 0.8, 0.9);
};
...
transposeWindows = function(swapFrame, switchFocus) {
  ...
  if (!((targets != null ? targets.length : void 0) > 0)) {
    Phoenix.alert("Can't see any windows to transpose");
    return;
  }
...
App.focusOrStart = function(title) {
  ...
  if (_.isEmpty(apps)) {
    Phoenix.alert("Attempting to start " + title);
...
key_binding = function(key, modifier, fn) {
  return Phoenix.bind(key, modifier, fn);
};

mash = 'cmd+alt+shift+ctrl'.split('+');


key_binding('C', mash, function() {
  return Window.focusedWindow().toMidCenter();
});
...
key_binding("`", mash, function() {
  return Phoenix.runCommand("/usr/bin/open", ["https://gist.githubusercontent.com/jasonm23/4990cc1e02a3c2a8e159/raw/phoenix.keyboard.png"]);
});
